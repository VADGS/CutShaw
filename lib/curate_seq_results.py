...
class CutShaw:
    ...
    def seq_results(self):
        ...
        fastani_taxons = fastani_obj.fastani()

        if not os.path.isfile(seq_results_out):
            ...
            for read in self.runfiles.reads:
                ...
                fastani_obj = run_fastani.FastANI(path=self.path, output_dir=self.output_dir)
                fastani_taxon = fastani_taxons[0][id]
                fastani_reference = fastani_taxons[1][id].replace(".fasta", "")
            ...
            print(f"Seq results exists at {seq_results_out}")
